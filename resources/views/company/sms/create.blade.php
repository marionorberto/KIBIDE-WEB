...
@section('content')
  ...
  <div class="col-xl-6">
    ...
    <div class="alert alert-success" role="alert"> <i class="ti ti-check"></i> {{ $successMessage }} <strong><a
      href="{{ route('company.sms.sent') }}">Ver Enviadas!</a></strong></div>
    @endif

    <div class="card">
    <form method="post" action="{{ route('company.sms.store') }}" class="card-body">
      @csrf
      ...
      <select class="form-select" id="type-user" required>
        ...
        <option value="manager">Usuário Gerente</option>
        <option value="superadmin">Apoio Técnico(KIBIDE)</option>
      </select>
  ...
  <script>
    ...
    if (typeUser.value == 'manager') {
      // selectReceivers.disabled = false;

      fetch(`/api/user/${companyId}/managers`)
      .then((res) => {
        if (res.ok) {
        return res.json();
        }

        throw new Error('erro trying to get the all managers by users');
      })
      .then((data) => {
    ...
    } else if (typeUser.value == 'superadmin') {


      fetch(`/api/user/superadmin/data`)
      .then((res) => {
        if (res.ok) {
        return res.json();
        }

        throw new Error('erro trying to get the superadmin informations');
      })
      .then((data) => {
